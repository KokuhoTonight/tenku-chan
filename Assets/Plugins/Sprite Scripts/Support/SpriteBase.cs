...
public class UVAnimation
{
	...
	public JSONObject ToJSON() {
		return new JSONParser().Deparse(this);
	}
	
	public static UVAnimation FromJSON(JSONObject json) {
		return new JSONParser().Parse(json);
	}
	
	public class JSONParser : IJSONParser< UVAnimation > {
		public UVAnimation Parse(JSONObject json) {
			UVAnimation uva = new UVAnimation();

			uva.name = json["name"].stringValue;
			uva.loopCycles = json["loop_cycles"].intValue;
			uva.loopReverse = json["loop_reverse"].boolValue;
			uva.framerate = json["framerate"].floatValue;
			uva.index = json["index"].intValue;
			uva.onAnimEnd = (ANIM_END_ACTION)Enum.Parse(typeof(ANIM_END_ACTION), json["on_anim_end"].stringValue);
			uva.frames = json["frames"].ToArray(new SPRITE_FRAME.JSONParser() );
			uva.CalcLength();
			
			return uva;
		}
		
		public JSONObject Deparse( UVAnimation uva) {
			JSONObject json = new JSONObject();

			json["name"] = new JSONObject(uva.name);
			json["loop_cycles"] = new JSONObject(uva.loopCycles);
			json["loop_reverse"] = new JSONObject(uva.loopReverse);
			json["framerate"] = new JSONObject(uva.framerate);
			json["index"] = new JSONObject(uva.index);
			json["on_anim_end"] = new JSONObject(uva.onAnimEnd.ToString());
			
			json["frames"].SetArray( uva.frames, new SPRITE_FRAME.JSONParser()); 
			return json;
		}
	}
