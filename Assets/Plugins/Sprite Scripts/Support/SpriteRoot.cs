...
public struct SPRITE_FRAME
{
	...
	public string ToJSON() {
		return new JSONParser().Deparse(this).ToString();
	}
	
	public static SPRITE_FRAME FromJSON(string text) {
		return new JSONParser().Parse(JSONObject.Read(text));
	}
	
	public class JSONParser : IJSONParser< SPRITE_FRAME > {
		public SPRITE_FRAME Parse(JSONObject json) {
			SPRITE_FRAME sf = new SPRITE_FRAME();

			sf.uvs.x = json["uvs_x"].floatValue;
			sf.uvs.y = json["uvs_y"].floatValue;
			sf.uvs.width = json["uvs_width"].floatValue;
			sf.uvs.height = json["uvs_height"].floatValue;
			sf.scaleFactor.x = json["scale_factor_x"].floatValue;
			sf.scaleFactor.y = json["scale_factor_y"].floatValue;
			sf.topLeftOffset.x = json["top_left_offset_x"].floatValue;
			sf.topLeftOffset.y = json["top_left_offset_y"].floatValue;
			sf.bottomRightOffset.x = json["bottom_right_offset_x"].floatValue;
			sf.bottomRightOffset.y = json["bottom_right_offset_y"].floatValue;

			return sf;
		}
		
		public JSONObject Deparse(SPRITE_FRAME sf) {
			JSONObject json = new JSONObject();
			json["uvs_x"] = new JSONObject(sf.uvs.x);
			json["uvs_y"] = new JSONObject(sf.uvs.y);
			json["uvs_width"] = new JSONObject(sf.uvs.width);
			json["uvs_height"] = new JSONObject(sf.uvs.height);
			json["scale_factor_x"] = new JSONObject(sf.scaleFactor.x);
			json["scale_factor_y"] = new JSONObject(sf.scaleFactor.y);
			json["top_left_offset_x"] = new JSONObject(sf.topLeftOffset.x);
			json["top_left_offset_y"] = new JSONObject(sf.topLeftOffset.y);
			json["bottom_right_offset_x"] = new JSONObject(sf.bottomRightOffset.x);
			json["bottom_right_offset_y"] = new JSONObject(sf.bottomRightOffset.y);
...
//			uvRect.yMax = frameInfo.uvs.y + bleedCompensationUVMax.y + (frameInfo.uvs.height) * tlTruncate.y * topClipPct;
			Rect baseUV = Rect.MinMaxRect(frameInfo.uvs.x + bleedCompensationUV.x, frameInfo.uvs.y + bleedCompensationUV.y, frameInfo.uvs.xMax - bleedCompensationUV.x, frameInfo.uvs.yMax - bleedCompensationUV.y);
			uvRect.x = Mathf.Lerp(baseUV.xMax, baseUV.x, tlTruncate.x * leftClipPct);
			uvRect.y = Mathf.Lerp(baseUV.yMax, baseUV.y, brTruncate.y * bottomClipPct);
			uvRect.xMax = Mathf.Lerp(baseUV.x, baseUV.xMax, brTruncate.x * rightClipPct);
			uvRect.yMax = Mathf.Lerp(baseUV.y, baseUV.yMax, tlTruncate.y * topClipPct);
//			uvRect = baseUV;
		}
		else if(clipped)
		{
...
// 			return true;
	...
	protected void DestroyMesh()
	{
		...
		DestroyImmediate(((SpriteMesh)m_spriteMesh).mesh);
		m_spriteMesh = null;
	...
	public virtual void Mirror(bool mirror){
		if(mirror){
			offset = new Vector3(-originalOffset.x, originalOffset.y, originalOffset.z);
			SetSize(-Mathf.Abs(width), height);
		}else{
			offset = new Vector3(originalOffset.x, originalOffset.y, originalOffset.z);
			SetSize(Mathf.Abs(width), height);
		}
