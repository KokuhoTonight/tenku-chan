...
public class PackedSprite : AutoSpriteBase
{
	...
	public static PackedSprite FromJSON(JSONObject json) {
		return new JSONParser().Parse(json);
	}
	
	public class JSONParser : IJSONParser< PackedSprite > {
		public PackedSprite Parse(JSONObject json) {
			PackedSprite sprite = PackedSprite.Create("test", Vector3.zero);
			
			UVAnimation[] anims = json["animations"].ToArray(new UVAnimation.JSONParser());
			foreach(UVAnimation anim in anims){
		...
		public JSONObject Deparse( PackedSprite sprite) {
			JSONObject json = new JSONObject();
			json["offset_x"] = new JSONObject(sprite.offset.x);
			json["offset_y"] = new JSONObject(sprite.offset.y);
			json["offset_z"] = new JSONObject(sprite.offset.z);
			json["static_frame"] = (new SPRITE_FRAME.JSONParser()).Deparse(sprite.staticFrameInfo);//new JSONObject(staticFrameInfo.ToJSON());
			json["animations"].SetArray(sprite.animations, new UVAnimation.JSONParser());
			json["anchor"] = new JSONObject(sprite.anchor.ToString());
			return json;
		}
	}
}
