...
public class UIScrollList : MonoBehaviour, IUIObject
{
	...
	protected void Awake()
	{
		...
		if(rig == null) rig = gameObject.AddComponent<Rigidbody>();
...
*/
	...
	public void RemoveItems(IUIListObject[] deletions, bool destroy)
	{
		for(int l = 0; l < deletions.Length; l++)
		{
			for(int i=0; i<items.Count; ++i)
			{
				if(items[i] == deletions[l])
				{
					RemoveItem(i, destroy, false, false);
					break;
				}
			}
		}
	...
	public void RemoveItem(int index, bool destroy, bool doEasing, bool reposition = true)
	{
		if (index < 0 || index >= items.Count)
			return;
