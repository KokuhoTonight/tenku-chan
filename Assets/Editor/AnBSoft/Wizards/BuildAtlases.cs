...
public class MaterialSpriteList
{
	...
	public JSONObject ToJSON(Object[] objs) {
		JSONObject json = new JSONObject();
		Debug.Log(objs.Length);
		Debug.Log(sprites.Count);
//		foreach(ISpriteAggregator s in sprites){
//			Debug.Log(s.SpriteFrames.Length);
//			Debug.Log(s.SourceTextures.Length);
//		}
//		Debug.Log(((ISpriteAggregator)(sprites[0])).SpriteFrames.Length);
//		Debug.Log(((ISpriteAggregator)(sprites[0])).SourceTextures.Length);
		for(int j=0; j<sprites.Count; ++j){
			for(int i=0; i<((ISpriteAggregator)(sprites[j])).SpriteFrames.Length; ++i){
				if(!((ISpriteAggregator)(sprites[j])).SourceTextures[i]){
					continue;
				}
				SPRITE_FRAME frame = new SPRITE_FRAME();
				frame.Copy((((ISpriteAggregator)(sprites[j])).SpriteFrames[i]));
				json[((ISpriteAggregator)(sprites[j])).SourceTextures[i].name] = (new SPRITE_FRAME.JSONParser()).Deparse(frame);
			}
		}
		return json;
	}
...
public class BuildAtlases : ScriptableWizard
{
	...
	public void OnWizardCreate()
	{
		...
		if(saveJSON){
			string path;
			for (int i = 0; i < matSpriteMap.Count; ++i){
				path = "Assets/StreamingAssets/Debug/Textures/"+matSpriteMap[i].material.name+".json";
				System.IO.StreamWriter writer = new System.IO.StreamWriter(path, false);
				writer.WriteLine(matSpriteMap[i].ToJSON((Object[])sprites.ToArray(typeof(Object))).ToString());
				writer.Close();
			}
			AssetDatabase.Refresh();
		}
	...
	//	o = FindObjectsOfType(typeof(Internationalizer));
